...
import type { CodeQuality as CodeQualityType } from "@/lib/ai/schemas";

interface CodeQualityProps {
  overallAssessment: string;
  findings: CodeQualityType["findings"];
}

export function CodeQuality({ overallAssessment, findings }: CodeQualityProps) {
  // Initialize expanded state: critical findings are open by default, or the first one if none are critical
  ...
  return (
    <div className="space-y-6 w-full max-w-full overflow-hidden">
      <div className="space-y-3">
        <h3 className="text-base font-semibold text-foreground/80">Quality & Best Practices Assessment</h3>
      ...
      <div>
        <h4 className="text-sm font-semibold mb-4 text-foreground/85">Quality Findings</h4>
        {findings.length === 0 ? (
          <p className="text-sm text-muted-foreground">Excellent code quality; no issues found.</p>
        ) : (
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-2.5 truncate">
                      ...
                      />
                      <Gem className="h-4 w-4 text-emerald-500 shrink-0" />
                      <span className="font-sans text-xs font-semibold text-foreground/90 truncate">
                        <span className="text-[10px] text-muted-foreground font-mono font-medium mr-1.5 bg-muted px-1.5 py-0.5 rounded border border-border/40 uppercase">
                          {finding.category}
                        </span>
                        {finding.title}
                      </span>
                  ...
                  {isExpanded && (
                    <div className="border-t border-border/20 bg-card/40 p-4 space-y-3 text-xs w-full max-w-full overflow-hidden">
                      ...
                      <div className="text-xs text-cyan-700 dark:text-cyan-400 pt-2.5 border-t border-border/10 leading-relaxed w-full max-w-full">
                        <strong className="text-foreground/75 dark:text-foreground/95">Suggestion:</strong>{" "}
                        {finding.suggestion}
                      </div>
                    </div>
                  )}
